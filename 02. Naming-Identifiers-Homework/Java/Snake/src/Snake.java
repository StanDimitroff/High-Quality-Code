...
public class Snake {
	...
	public void drawSnake(Graphics graph) {		
		for (Point point : this.snakeBody) {
			point.drawSnake(graph, snakeColor);
		}
	}
	
	public void tick() {
		Point current = new Point((snakeBody.get(0).getX() + velocityX), (snakeBody.get(0).getY() + velocityY));
		...
		if (current.getX() > Game.WIDTH - 20) {
			...
			Game.gameRunning = false;
		} else if (Game.apple.getPoint().equals(current)) {
